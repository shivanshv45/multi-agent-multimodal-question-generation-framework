# Groq API integration (Uses OpenAI SDK format)
...
class GroqBackend(ModelBackend):
    ...
    def initialize(self) -> None:
        ...
        self._client = OpenAI(api_key=self.api_key, base_url=self.GROQ_BASE_URL)
    ...
    async def generate(self, prompt, system_prompt=None, temperature=0.7, max_tokens=4096, json_mode=False) -> BackendResponse:
        ...
        return BackendResponse(text=response.choices[0].message.content or "", model=self.model, backend="groq", usage=usage, raw_response=response)
    ...
    async def generate_with_image(self, prompt, image_path, system_prompt=None, temperature=0.7, max_tokens=4096, json_mode=False) -> BackendResponse:
        ...
        if "vision" not in self.model.lower() and "qwen" not in self.model.lower():
            vision_model = "qwen/qwen3.6-27b" # Fallback to a vision model for Groq if not specified
        ...
        return BackendResponse(text=response.choices[0].message.content or "", model=vision_model, backend="groq", usage=usage, raw_response=response)
