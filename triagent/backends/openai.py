...
class OpenAIBackend(ModelBackend):
    ...
    def initialize(self) -> None:
        ...
        if not self.api_key:
            raise ValueError("OPENAI_API_KEY not set. Get one at https://platform.openai.com/")
    ...
    async def generate(self, prompt, system_prompt=None, temperature=0.7, max_tokens=4096, json_mode=False) -> BackendResponse:
        ...
        return BackendResponse(text=response.choices[0].message.content or "", model=self.model, backend="openai", usage=usage, raw_response=response)
